...
public enum NotificationType {
    ...
    PLAYLIST_SUBSCRIBED("%1$s님이 구독을 시작했습니다.", "%2$s", Level.INFO),
    ...
    FOLLOWING_ACTIVITY_PLAYLIST("%1$s님이 플레이리스트를 만들었어요.", "[%2$s] %3$s", Level.INFO),

    // 다른 사용자가 나를 팔로우
    FOLLOW_ME("팔로우 알림", "%s님이 나를 팔로우했어요.", Level.INFO),

    // DM 수신 (title: 발신자 이름, body: 메시지 내용)
    DM_RECEIVED("[DM] %1$s", "%2$s", Level.INFO),
    ;
